...
<h1 class="text-9 font-bold mb-5">Warna</h1>
<div class="w-full flex flex-col">  
    <div class="flex mb-7 self-end">
        <a href="/tambah_warna" class="flex gap-1 bg-pink-1 transition-colors hover:bg-pink-2 items-center text-white rounded-1 font-monda font-medium py-2 px-4">
            Tambah Warna
    ...
    <table class="w-full text-left">
        ...
        <tbody class="divide-y divide-gray-200">
            <?php foreach ($warnatryon as $index => $p) : ?>
                <tr class="<?= $index % 2 === 0 ? 'bg-white' : 'bg-white' ?> border-b mr-6 bg-pink-1">
                    <th scope="row" class="px-6 py-4 font-medium text-black whitespace-nowrap truncate max-w-[100px]">
                        <img src="/img/<?= $p['gambar_orang']; ?>" alt="" class="w-3/4">
                    </th>
                    <td class="px-6 py-4 font-medium text-black truncate max-w-[100px]">
                        <?= $p['nama_warna']; ?>
                    </td>
                    <td class="px-6 py-4 font-medium text-black truncate max-w-[100px]">
    <div style="background-color: <?= $p['kode_warna']; ?>; width: 170px; height: 100px;"></div>
    <?= $p['kode_warna']; ?>
</td>
                    <td class="px-6 py-4">
                        <div class="flex justify-center">
                            <div class="flex justify-center">
                                <a class="mr-2" href="/edit_warna/<?= $p['id_warna']; ?>">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-blue-600">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>
                                </a>
                                <a class="mr-2" href="/hapus_warna/<?= $p['id_warna']; ?>">
